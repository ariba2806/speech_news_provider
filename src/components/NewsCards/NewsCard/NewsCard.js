...
const NewsCard = ({ article: { description, publishedAt, source, title, url, urlToImage }, activeArticle, i }) => {
  ...
  return (
    <Card style={{boxShadow: '7px 7px 8px grey'}} ref={elRefs[i]} className={ activeArticle === i ? classes.activeCard : classes.card}>
      ...
      <CardActions className={classes.cardActions}>
        <Button size="small" color="primary" href={url}>Read More</Button>
        <Typography color="textSecondary" component="h3">Article {i + 1}</Typography>
      </CardActions>
    </Card>
  );
};
